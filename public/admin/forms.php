...
$module = $_GET['module'] ?? '';
$sub = $_GET['submodule'] ?? '';
$formPath = $_SERVER['DOCUMENT_ROOT'] . "/erp/modules/$module/$sub/form.json";

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $newSchema = $_POST['schema'] ?? '';
    file_put_contents($formPath, $newSchema);
    header("Location: forms.php?module=$module&submodule=$sub&saved=1");
    exit;
}

$currentSchema = file_exists($formPath) ? file_get_contents($formPath) : '[]';
?>
<!DOCTYPE html>
<html>
<head>
    <title>Edit Form Schema</title>
    <link href="https://cdn.jsdelivr.net/npm/tailwindcss@2.2.19/dist/tailwind.min.css" rel="stylesheet">
</head>
<body class="p-6">
    <h1 class="text-2xl font-bold mb-4">Edit Form: <?= htmlspecialchars("$module / $sub") ?></h1>
