...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($user && password_verify($password, $user['password'])) {
        $_SESSION['user'] = $user;
        header("Location: " . ($email === 'admin@example.com' ? 'admin/index.php' : 'index.php'));
        exit;
    } else {
        $error = "Invalid login.";
    }
...
<body class="bg-gray-100 flex justify-center items-center h-screen">
    <form method="POST" class="bg-white p-8 shadow rounded w-96">
        ...
        <?php endif; ?>
        <input name="client" placeholder="Client ID" class="w-full mb-4 p-2 border rounded" <?= isset($error) ? '' : 'required' ?> />
